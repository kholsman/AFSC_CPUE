...
compare_plot <- function(obsIN = obs,cpue1, cpue2, spIN="plk",offset=0){
  ...
  p    <- ggplot()+
    ...
    geom_point(data=tmp,
               aes(x=YEAR+2*offset,y=totB*1000, color ="CEATTLE (vast?) NEBS+SEBS"),size=2)+
    geom_errorbar(data=tmp,
               aes(x=YEAR+2*offset,ymin=(totB-totBse)*1000,ymax=(totB+totBse)*1000, color ="CEATTLE (vast?) NEBS+SEBS"))+theme_minimal()+ggtitle(spnm)
  return(p)
}
